...
public class LeaveFormController extends BaseController{
    ...
    public String edit(@PathVariable("procInstId") String procInstId, Model model)
    {
        ...
        Integer sample_id = leaveForm.getTitle();
        String sample_name = sampleService.selectByPrimaryKey(sample_id).getTitle();
        List<User> users = iUserService.selectByUser(new User());
        model.addAttribute("users", users);
        model.addAttribute("Form", leaveForm);
        model.addAttribute("sname", sample_name);
    ...
    @Operlog(modal = "样品申请管理", descr = "修改样品申请表单")
    @ResponseBody
     ...
     */
    @RequestMapping("/del")
    @Operlog(modal = "样品申请管理", descr = "删除样品申请记录")
    @ResponseBody
    ...
    public TableDataInfo myLeavRecordHi(ActHiProcinst actHiProcinst)
    {
        ...
        actHiProcinst.setStartUserId(getUserId());
