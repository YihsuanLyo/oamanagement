...
import static com.yj.oa.project.controller.ProductController.getFileBean;

@Controller
@RequestMapping("/product/add/uploadProduct")
public class UploadProductController {
    ...
    @ResponseBody
    public AjaxResult upload(@RequestParam("file") MultipartFile product, Product fileBean) {


        // 获取文件名
        String fileName = product.getOriginalFilename();
        if (!fileName.isEmpty()) {
            ...
            try {
                product.transferTo(new File(uploadPath+"/"+filename));// 文件写入
            ...
            iProductService.insertSelective(getFileBean(product, filename, fileBean,getUser().getName()));
            return success();
