...
import com.yj.oa.project.service.chain.IChainService;
import com.yj.oa.project.service.hardware.IHardwareService;
...
public class HardwareController extends BaseController{
    ...
    private String prefix = "system/hardware/";
    ...
    @ResponseBody
    public TableDataInfo listPag(@PathVariable("programId") Integer programId, Hardware hardware, Model model)
    {
        startPage();
        hardware.setProgramId(programId);
        List<Hardware> certificates = iHardwareService.selectHardwareList(hardware);
        return getDataTable(certificates);
    }
    ...
    @ResponseBody
    public AjaxResult addSave(MultipartFile file, Hardware fileBean)
    {
        if (file.isEmpty())
        {
            return error("请选择文件！");
       ...
       // 上传
        ...
        iHardwareService.insertSelective(getFileBean(file, fileId, fileBean,getUser().getName()));
    ...
    public AjaxResult del(String[] ids)
    {
        ...
        //2.删除数据库
        int i = iHardwareService.deleteByPrimaryKeys(ids);
    ...
    public ResponseEntity<byte[]> download(String fileId, String fileName)
    {
        try
        {
           Hardware files = new Hardware();
            files.setFileId(fileId);
            iHardwareService.downloadCountAddOne(files);
     ...
     */
    public static Hardware getFileBean(MultipartFile file, String fileId, Hardware fileBean,String userName)
    {
