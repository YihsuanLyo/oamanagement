...
import static com.yj.oa.project.controller.HardwareController.getFileBean;

@Controller
@RequestMapping("/hardware/add/uploadHardware")
public class UploadHardwareController {

    private static final Logger log = LoggerFactory.getLogger(UploadHardwareController.class);
    ...
    @ResponseBody
    public AjaxResult upload(@RequestParam("file") MultipartFile design, Hardware fileBean) {
        ...
        if (!fileName.isEmpty()) {
            ...
            iHardwareService.insertSelective(getFileBean(design, filename, fileBean,getUser().getName()));
            return success();
