...
public class CertificateController extends BaseController{
    ...
    @ResponseBody
    public TableDataInfo listPag(@PathVariable("programId") Integer programId, Certificates certificates, Model model)
    {
        startPage();
        certificates.setProgramId(programId);
        List<Certificates> certificatess = iCertificateService.selectCertificateList(certificates);
        return getDataTable(certificatess);
    }
    ...
    @ResponseBody
    public AjaxResult addSave(MultipartFile file, Certificates fileBean)
    {
        if (file.isEmpty())
        {
            return error("请选择文件！");
       ...
       // 上传
        ...
        iCertificateService.insertSelective(getFileBean(file, fileId, fileBean,getUser().getName()));
    ...
    public AjaxResult del(String[] ids)
    {
        ...
        //2.删除数据库
        int i = iCertificateService.deleteByPrimaryKeys(ids);
    ...
    public ResponseEntity<byte[]> download(String fileId, String fileName)
    {
        try
        {
            Certificates files = new Certificates();
            files.setFileId(fileId);
            iCertificateService.downloadCountAddOne(files);
     ...
     */
    public static Certificates getFileBean(MultipartFile file, String fileId, Certificates fileBean,String userName)
    {
