...
public class ChainController extends BaseController{
    ...
    @ResponseBody
    public TableDataInfo listPag(@PathVariable("programId") Integer programId, Chain chain, Model model)
    {
        startPage();
        chain.setProgramId(programId);
        List<Chain> certificates = iChainService.selectChainList(chain);
    ...
    @ResponseBody
    public AjaxResult addSave(MultipartFile file, Chain fileBean)
    {
        if (file.isEmpty())
        {
            return error("请选择文件！");
       ...
       // 上传
        ...
        iChainService.insertSelective(getFileBean(file, fileId, fileBean,getUser().getName()));
    ...
    public AjaxResult del(String[] ids)
    {
        ...
        //2.删除数据库
        int i = iChainService.deleteByPrimaryKeys(ids);
    ...
    public ResponseEntity<byte[]> download(String fileId, String fileName)
    {
        try
        {
            Chain files = new Chain();
            files.setFileId(fileId);
            iChainService.downloadCountAddOne(files);
     ...
     */
    public static Chain getFileBean(MultipartFile file, String fileId, Chain fileBean,String userName)
    {
