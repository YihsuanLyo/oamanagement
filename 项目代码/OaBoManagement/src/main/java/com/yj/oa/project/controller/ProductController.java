...
import com.yj.oa.project.service.product.IProductService;
...
public class ProductController extends BaseController{
    ...
    @ResponseBody
    public TableDataInfo listPag(@PathVariable("programId") Integer programId, Product product, Model model)
    {
        startPage();
        product.setProgramId(programId);
        List<Product> products = iProductService.selectProductList(product);
    ...
    @ResponseBody
    public AjaxResult addSave(MultipartFile file, Product fileBean)
    {
        if (file.isEmpty())
        {
            return error("请选择文件！");
       ...
       // 上传
        ...
        iProductService.insertSelective(getFileBean(file, fileId, fileBean,getUser().getName()));
    ...
    public AjaxResult del(String[] ids)
    {
        ...
        //2.删除数据库
        int i = iProductService.deleteByPrimaryKeys(ids);
    ...
    public ResponseEntity<byte[]> download(String fileId, String fileName)
    {
        try
        {
            Product files = new Product();
            files.setFileId(fileId);
            iProductService.downloadCountAddOne(files);
     ...
     */
    public static Product getFileBean(MultipartFile file, String fileId, Product fileBean,String userName)
    {
