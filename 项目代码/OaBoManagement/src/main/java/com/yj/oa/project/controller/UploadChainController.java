...
import static com.yj.oa.project.controller.ChainController.getFileBean;

@Controller
@RequestMapping("/chain/add/uploadChain")
public class UploadChainController {
    ...
    @Autowired
    private IChainService iChainService;

    @RequestMapping(value = "/chain",method = RequestMethod.POST)
    @ResponseBody
    public AjaxResult upload(@RequestParam("file") MultipartFile certificate, Chain fileBean) {
        ...
        if (!fileName.isEmpty()) {
            ...
            iChainService.insertSelective(getFileBean(certificate, filename, fileBean,getUser().getName()));
            return success();
